...
void update(ll i, int n, int val,VL &bit){
    while(i<=n){
          bit[i]+=val;
          i+=i&-i;
    }
}
ll query(ll i, VL &bit){
    ll sum=0;
    while(i>0){
        sum+=bit[i];
        i-=i&-i;
...
ll range(int l, int r , VL &bit){
    if(l>=r) return 0;
    return query(r,bit)-query(l,bit);
}
...
void solve()
{
    ...
    ll ans=0;
    for(int i=0; i<n; i++){
        int idx=upper_bound(all(srt),v[i])-srt.begin();
        ans+=range(idx,m,bit);
        update(idx,m,1,bit);
    }
    cout<<ans<<'\n';
